// =====================================================
// EasyTube AdBlock - AdGuard + Banery Sponsorowane
// =====================================================
...
(function() {
    ...
    if (!adblockEnabled) {
        ...
        const elements = document.querySelectorAll('#block-youtube-ads-logo, #block-youtube-ads-style, #easytube-extra-adblock, #easytube-sponsored-blocks');
        elements.forEach(el => el.remove());
        return;
    }
    
    // =====================================================
    // ORYGINALNY AdGuard - BLOKOWANIE REKLAM W FILMACH
    // =====================================================
    
    function runAdGuard() {
        try {
            if (!document || !document.head) {
                setTimeout(runAdGuard, 200);
                return;
            }
            
            // Oryginalny kod AdGuard
            const locales = {
                en: {
                    logo: "with&nbsp;AdGuard",
...
.ytmusic-nav-bar#left-content #${LOGO_ID} { display: block; }`;
    ...
    function blockSponsoredBanners() {
        try {
            if (!document || !document.head) {
                setTimeout(blockSponsoredBanners, 300);
                return;
            }
            
            // Style dla banerów sponsorowanych
            const sponsoredStyles = document.createElement('style');
            sponsoredStyles.id = 'easytube-sponsored-blocks';
            sponsoredStyles.textContent = `
                /* Blokowanie banerów sponsorowanych na stronie głównej */
            ...
            `;
            document.head.appendChild(sponsoredStyles);
